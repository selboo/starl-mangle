...
def remote_exec(host, port, command):
    ...
    sock.send(encryption(command))

    tcp_head   = sock.recv(100).split('|selboo')[0].split(',')
    tcp_length = int(tcp_head[0])
    tcp_subcon = int(tcp_head[1])

    alist = []

    if tcp_length <= 100:
        alist.append(sock.recv(100))
        data = "".join(alist)
        sock.close()
        return data
