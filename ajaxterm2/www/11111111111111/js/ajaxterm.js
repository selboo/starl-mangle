...
ajaxterm.Terminal_ctor=function(id,width,height,userip,port,type,pk) {
	var ie=0;
	if(window.ActiveXObject)
		ie=1;
	var sid=""+Math.round(Math.random()*1000000000);
	var query0="s="+sid+"&w="+width+"&h="+height+"&i="+userip+"&p="+port+"&t="+type;	var query1=query0+"&c=1&k=";
	var query1=query0+"&c=1&k=";
	var buf="";
	var timeout=600;
	var error_timeout=600;
	var keybuf=[];
	var sending=0;
	var rmax=1;
    var keystate = -1;
	...
	function update() {
		if(sending == 0) {
			...
			error_timeout = window.setTimeout(error,error_timeout);
			if(opt_get.className == 'on') {
				r.send(null);
			} else {
				r.send(query);
...
//}
ajaxterm.Terminal=function(id,width,height,user,ip,port,type,pk) {
	return new this.Terminal_ctor(id,width,height,user + "@" + ip,port,type,pk);
}
